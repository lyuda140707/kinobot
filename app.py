...
async def referral_join(request: Request):
    """
    Викликається, коли новий користувач заходить по посиланню t.me/bot?start=ref_<user_id>.
    """
    data = await request.json()
    ...
    if not referrer_id or not new_user_id:
        raise HTTPException(status_code=400, detail="referrer_id або new_user_id відсутні")

    service = get_google_service()
    sheet = service.spreadsheets()
    sheet_id = os.getenv("SHEET_ID")

    # читаємо таблицю Referrals
    rows = sheet.values().get(
        spreadsheetId=sheet_id,
        range="Referrals!A2:C1000"
    ).execute().get("values", [])

    found = False
    for idx, row in enumerate(rows, start=2):
        if row and row[0] == referrer_id:
            count = int(row[1]) if len(row) > 1 and row[1].isdigit() else 0
            count += 1
            sheet.values().update(
                spreadsheetId=sheet_id,
                range=f"Referrals!B{idx}",
                valueInputOption="RAW",
                body={"values": [[str(count)]]}
            ).execute()
            found = True

            # якщо вже є 3 запрошених — даємо 7 днів PRO
            if count >= 3:
                from pro_utils import add_pro_user
                add_pro_user(referrer_id, days=7)
                sheet.values().update(
                    spreadsheetId=sheet_id,
                    range=f"Referrals!C{idx}",
                    valueInputOption="RAW",
    ...
    if not found:
        sheet.values().append(
            ...
            valueInputOption="USER_ENTERED",
            insertDataOption="INSERT_ROWS",
            body={"values": [[referrer_id, "1", ""]]}
        ).execute()

    return {"ok": True}
@app.post("/referral-count")
async def referral_count(request: Request):
    data = await request.json()
    user_id = str(data.get("user_id"))
