...
async def referral(request: Request):
    data = await request.json()
    inviter_id = str(data.get("inviter_id"))   # хто запросив
    friend_id = str(data.get("friend_id"))     # новий користувач

    if not inviter_id or not friend_id:
        return JSONResponse(status_code=400, content={"error": "inviter_id або friend_id відсутні"})

    # не дозволяємо запрошувати самого себе
    if inviter_id == friend_id:
        return {"success": False, "error": "Не можна запросити самого себе"}
    ...
    if any(r[0] == inviter_id and r[1] == friend_id for r in existing if len(r) >= 2):
        return {"success": False, "error": "Вже зараховано"}
    ...
    sheet.values().append(
        spreadsheetId=SHEET_ID,
        range="Referral!A2:C2",
        valueInputOption="USER_ENTERED",
        body={"values": [[inviter_id, friend_id, now]]}
    ).execute()

    # перевіряємо кількість унікальних друзів
    friends = [r[1] for r in existing if len(r) >= 2 and r[0] == inviter_id]
    friends.append(friend_id)
    unique_count = len(set(friends))

    if unique_count >= 3:
        # ✅ даємо PRO на 7 днів
        expire_date = (datetime.now(kyiv) + timedelta(days=7)).strftime("%Y-%m-%d")
        sheet.values().append(
            spreadsheetId=SHEET_ID,
            range="PRO!A2:D2",
            valueInputOption="USER_ENTERED",
            body={"values": [[inviter_id, "", "Активно", expire_date]]}
        ).execute()
